import torch
from transformers import GPTBigCodeForCausalLM, PhiForCausalLM, LlamaForCausalLM, Qwen2ForCausalLM, CodeGenForCausalLM, MistralForCausalLM
...
class MistralPrefix(MistralForCausalLM):
    def __init__(self, config):
        super().__init__(config)
...
class CodeLlamaPrefix(LlamaForCausalLM):
    def __init__(self, config):
        ...
        for _ in range(2):
            for _ in range(config.num_hidden_layers):
                for _ in range(2):
                    param_size = (config.num_key_value_heads, 10, config.hidden_size // config.num_key_value_heads)
                    param = torch.nn.Parameter(torch.zeros(param_size, requires_grad=True))
                    self.prefix_params.append(param)
...
class CodeGenPrefix(CodeGenForCausalLM):
    def __init__(self, config):
        super().__init__(config)

        n_embed_per_head = config.n_embd // config.n_head
        self.prefix_params = torch.nn.ParameterList()

        for _ in range(2):
            for _ in range(config.n_layer):
                for _ in range(2):
                    param_size = (config.n_head, 10, n_embed_per_head)
                    param = torch.nn.Parameter(torch.zeros(param_size, requires_grad=True))
                    self.prefix_params.append(param)
